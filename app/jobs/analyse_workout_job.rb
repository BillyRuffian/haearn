class AnalyseWorkoutJob < ApplicationJob
  ...
  retry_on Ai::Client::TransientError, wait: :polynomially_longer, attempts: 3 do |job, error|
    WorkoutAnalysis.where(id: job.arguments.first, status: 'pending').update_all(
  ...
  def perform(analysis_id)
    ...
    if analysis.workout.finished_at == analysis.workout_finished_at
      owned(analysis, token).update_all(**result, status: 'completed', error_message: nil, processing_token: nil, updated_at: Time.current)
    else
      owned(analysis, token).update_all(status: 'failed', error_message: 'workout_changed', processing_token: nil, updated_at: Time.current)
    end
  rescue Ai::Client::TransientError => error
    log_failure(analysis, error)
    owned(analysis, token).update_all(status: 'pending', error_message: error.message, processing_token: nil, updated_at: Time.current)
    raise
  rescue StandardError => error
    log_failure(analysis, error)
    code = error.is_a?(Ai::Client::Error) || error.is_a?(Ai::InvalidResponse) ? error.message : error.class.name
    owned(analysis, token).update_all(status: 'failed', error_message: code, processing_token: nil, updated_at: Time.current) if analysis && token
  end
