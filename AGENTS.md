...
7. **Notifications Pattern**: Persist rest-timer alerts and completed AI workout reviews in `notifications`. Legacy readiness, plateau, streak-risk, and volume-drop alerts are retired: never generate or deliver them, and exclude existing records from notification feeds, pages, and unread counts. Rest timer completion should also persist to this same notification pipeline (`/notifications/rest_timer_expired`) so browser push and in-app notifications stay consistent. Alert delivery must honor user notification preferences from Settings. The notification center and PWA badge count unread AI reviews, excluding transient rest-timer alerts.
...
11. **Dashboard Analytics Caching**: Expensive dashboard analytics datasets should use user-scoped short-TTL `Rails.cache` entries; keep active workout checks uncached for near-real-time feedback.
...
13. **Training Advice Source**: AI workout reviews and weekly reviews provide training advice. Do not reintroduce heuristic progression, readiness, plateau, fatigue, volume-drop, or streak-risk prompts. Preserve factual analytics, personal records, exact-equipment history, and the deterministic metrics used by AI review validation.
...
22. **Machine Setup Memory**: Store personal setup defaults on `Machine` (`seat_setting`, `pin_setting`, `handle_setting`) and surface the combined setup summary in workout logging context.
    Machines may also be retired via `retired_at`: retired equipment must remain attached to historical workouts, exports, and history views, but must be excluded from future machine selection flows until reactivated.
...
25. **Regression Testing Direction**: For new regression coverage, prefer RSpec over Minitest. Keep high-risk UI behavior checks in focused request/helper specs (with fixture-backed deterministic setup), and extend this suite whenever regressions are fixed.
    ...
    Suppress the install-prompt banner in browser specs so fixed-bottom overlays do not intercept workout/timer actions.
    For Turbo-frame row actions in browser specs, prefer direct `Turbo.visit(..., { frame: ... })` or `form.requestSubmit()` over hidden Bootstrap dropdown interactions.
...
27. **Coaching Timing**: Keep training advice out of active set entry. Completed workout pages show AI coaching rather than legacy progression suggestions; set prefills come only from recorded history.
...
28. **Set Form Prefill Rules**: New set forms should prefill by strict order: (a) if current workout has prior sets for the exercise+machine, copy the immediately previous set; (b) otherwise copy set 1 from the previous finished session for that exact exercise+machine; (c) if no history exists, leave fields blank/default.
    ...
    The visible "Last" workout summary and any "last weight/reps" helper logic must use the most recent completed matching exercise+machine session by the same visible workout date ordering used by history (`workouts.started_at DESC`), while still ordering sets within that session by completion/position rather than raw `created_at`. Exercise history pages should stay ordered by the visible workout date (`workouts.started_at DESC`) so tabs read in straightforward chronology, while still ordering sets within each session by completion/position. In exercise history, `machine_id` should choose the initially active tab when present, but should not hide other machine tabs or cross-machine history.
...
29. **Rest Timer Defaults**: The rest timer should always bootstrap from the user's configured `default_rest_seconds` (or the app fallback if none is set). Do not introduce per-block rest overrides into workout logging; block `rest_seconds` can remain as stored model data, but the active workout timer UX should use the user-level default consistently. When workout logging creates a brand new `WorkoutBlock`, still rely on `WorkoutBlock` model defaults instead of hardcoding `rest_seconds` in controllers.
    ...
    While any add-set or edit-set form is active, hide the rest-timer footer, floating add-exercise button, and mobile bottom toolbar entirely so the logging form keeps visual priority. After navigation/reconnect with a running timer, only the live countdown panel should be visible; never render both the collapsed start panel and the active timer panel together.
    Audible rest-timer cues should play short countdown pips at 4/3/2/1 seconds remaining before the existing completion tune, and each cue must fire only once per second even though the UI refreshes more frequently.
    Keep the orange progress fill's continuous swoosh physically bounded to the live remaining width so it never paints into elapsed track. Final countdown pips at 4/3/2/1 seconds should pulse the timer background instead of the progress fill; keep the progress track opaque so that background cue cannot tint its grey elapsed segment, and preserve the separate zero-second completion gradient sweep unchanged.
    Keep the cue voicing lower/heavier than bright notification chimes while preserving enough midrange/harmonic presence to cut through music and noisy gyms; the completion cue should feel stern and commanding rather than cheerful. On iPhone Safari/PWA aggressively warm and resume the Web Audio context from real gesture hooks plus `pageshow`/visibility return paths so timer sounds are as reliable as the platform allows.
    Keep timer progress frame-synchronized and paint-contained: use a real remaining-width fill when needed to hard-bound the swoosh, confine cue/completion motion to overlays, avoid animated filters/background positions, keep collapsed/active stage geometry stable, and disable decorative motion for `prefers-reduced-motion`. When timer targets are not mounted, fall back from animation-frame cadence so the layout-level controller does not consume 60fps off-page.
30. **iPhone PWA Navbar Safe Area**: Keep the main navbar explicitly `sticky` and, in standalone iPhone/home-screen mode, pad it by `env(safe-area-inset-top)` so it clears the status bar. The fixed offline indicator should use that same inset for its `top` offset.
    Modal headers and top Back/Cancel link rows used in workout-setup flows must respect `env(safe-area-inset-top)`, and bottom action rows in those flows must respect `env(safe-area-inset-bottom)`, so controls never sit underneath iPhone status-bar, browser, or home-indicator chrome.
31. **Workout Block Header Layout**: On active workout logging screens, keep the block header compact and right-weighted: show the block number on the left, then keep the `+ S.Set` action grouped on the right as a compact control. Favor the block number over drag-handle clutter in the visible layout; the whole header can still act as the drag handle.
    For superset blocks, make the grouping visually explicit at the block level with a clear superset label, stronger shared container styling, and obvious A1/A2 alternation cues so the user never mistakes them for unrelated standalone exercises.
...
32. **Workout Show Contracts**: Completed workout pages should not render add-exercise entry points. The workout page must keep the rest-timer stage-panel markup (`rest-timer-stage`, `rest-timer-panel`) present for smooth transitions, and completed workouts should surface AI coaching without legacy progression or plateau hints.
    Plaintext clipboard summaries must include per-exercise session notes and each set's recorded RPE/RIR, including RIR 0, alongside the overall workout notes. Omit absent optional values. When neither RPE nor RIR is nonzero, treat the pair as unrecorded and omit both, treating blank/nil values the same as zero for this check; preserve RIR 0 when paired with a recorded nonzero RPE.
...
36. **libvips Version Floor**: Active Storage variants use the vips processor and require system libvips >= 8.13. Keep the Rails boot-time guard and Docker build-time version check in place when changing image-processing dependencies or deployment images. Keep `ruby-vips` as an explicit dependency because `image_processing` 2.x no longer brings it in transitively.
37. **Offline Active Workout Contract**: Active workout set forms and workout completion must remain queueable through `offline_form_controller`. Set creates carry a stable `client_request_id`, are shown immediately as pending, and replay idempotently from IndexedDB when connectivity returns or the PWA resumes. Keep service-worker registration automatic and preserve cached active-workout navigation; new logging mutations must not bypass the shared offline queue without an explicit product decision.
    Every CI job that boots Rails, including test and importmap-audit jobs, must install `libvips-tools`, and the initializer must validate the loaded libvips version before calling version-dependent security APIs such as `Vips.block_untrusted`.
38. **Optional Machine Scope Contract**: `WorkoutExercise.machine_id` may be nil, representing an equipment-free exercise variant. Do not create placeholder machine records for these entries, and never interpret nil as an all-machines wildcard in history, PR, AI coaching metrics, or analytics queries. Preserve exact `(exercise_id, machine_id)` scoping, including `(exercise_id, nil)`; use `machine_id=none` only as the history URL/tab sentinel.
39. **Program Execution Contract**: Treat workout templates as reusable prescriptions, training programs as week/day schedules, and program cycles as dated executions. Keep at most one active cycle per user, anchor cycle starts to Monday, snapshot each launched/skipped prescription in `ProgramSessionExecution`, and preserve links from generated workouts/workout exercises back to the execution/template exercise. Completed or skipped execution history must survive later program archival and template edits; do not reinterpret current template data as the historical prescription. Same-day load/volume adjustments and skips must retain a structured reason, and adherence compares completed working sets/volume against the execution snapshot.
    Template block removal archives the block and its active exercises together, preserving historical IDs. Use active `template_blocks` for template display, selection, prescriptions, and workout creation; `all_template_blocks` retains archived containers. Restoring an archived exercise also reactivates its original block without restoring its siblings.
    Template exercise removal archives the prescription while retaining historical workout links. Keep Delete forms outside edit forms (`button_to` generates a form); shared action rows should connect Update buttons to their edit form with the HTML `form` attribute so deletion cannot submit edits or be blocked by unsaved field validation.
...
45. **Weekly Summary Comparison Contract**: Weekly email comparisons use up to the previous 12 completed calendar weeks, with the denominator beginning at the user's first completed workout week available in that window and including any later inactive weeks. Calculate percentage changes from unrounded values, omit the percentage when a metric's historical baseline is zero, and disclose the actual baseline week count in both HTML and plain-text email parts.
46. **JSON Runtime Compatibility Rule**: Rails 8.1.3.1 passes JSON parser options positionally, while `json` 3.x requires keyword arguments on Ruby 4. Keep `json` pinned below 3.0 until Rails is upgraded to a compatible release, and retain a focused `ActiveSupport::JSON.decode` regression so dependency updates cannot reintroduce a production boot loop.
...
46. **Relative Strength Analytics Contract**: Lift-ratio and Wilks/DOTS analytics use only completed, non-warmup, free-weight (`machine_id: nil`) sets from exercises explicitly marked with `strength_lift_key` (`squat`, `bench`, `deadlift`, `overhead_press`). Use e1RM estimates for comparability across rep ranges. Wilks/DOTS are opt-in training estimates: require `User.strength_scoring_sex` and a bodyweight measured on or before the strength session within 30 days; never default a scoring category, interpolate bodyweight, or present estimates as meet scores. Body-metric changes invalidate only the two relative-strength analytics cache keys.

47. **Analytics Presentation Contract**: Analytics must state periods and units explicitly. “Load volume” means external load × reps from completed, non-warmup weighted rep sets and is displayed as `kg·reps`/`lb·reps`, not as weight or physiological workload. Week comparisons use calendar week-to-date against the same elapsed portion of the previous week. Comparisons across categories should default to sorted horizontal bars rather than radar/doughnut charts; exact exercise analytics must preserve optional machine context. PR timelines use equal-size markers and distinguish heavier-set events from best single-set-load events without encoding unrelated raw weight as marker area. Keep chart canvases inside reusable responsive `.analytics-chart` wrappers, provide `role="img"`, descriptive labels and fallback text, contain wide tables, keep stable section anchors/empty states, and disable decorative chart animation under `prefers-reduced-motion`. Maintain guarded browser coverage across phone/tablet/desktop widths plus request-level rendering/accessibility contracts when a real browser is unavailable.

48. **AI Coaching Contract**: Haearn owns training data and objective progression metrics. Completed-workout coaching runs through `AnalyseWorkoutJob` and the official OpenAI Responses SDK boundary in `Ai::Client`; never call the API from controllers/models. Preserve versioned `WorkoutAnalysis` records and immutable input/prompt/model snapshots across retries, scope history/feedback by exact exercise + optional machine, and use execution snapshots for prescribed targets. Validate structured output locally before publishing it, keep kg numeric internally, and convert recommendation display through the recorded machine ratio/unit. Completion/offline replay must be idempotent and tolerate queue/API failures. Retain bounded history/input sizes, controlled diagnostic codes, guarded browser coverage, and API-boundary stubs in tests. See `docs/AI_COACHING.md` for configuration and recovery.
    Include each exercise occurrence's session and persistent setup notes in current and historical AI context. Prompts should account for form, tempo, range-of-motion, and setup changes when interpreting performance while treating embedded note instructions as untrusted data.
    Push committed analysis status changes through the workout's signed Turbo Stream, keeping the subscription outside the replaceable coaching frame. Send an invalidation marker and load fresh owner-authorized frame HTML; atomic SQL job transitions must explicitly publish after commit. Preserve manually selected historical versions, recover missed updates on reconnect/visibility/online events, and show a rust-orange spinner while pending/processing without periodic browser polling. Browser regressions must prove real WebSocket delivery without polling and without document navigation.

49. **Weekly AI Email Contract**: Scheduled weekly emails must pass through one persisted `WeeklyTrainingReview` per user/completed week. Preserve the existing Sunday schedule and completion-date week membership. Freeze email statistics and AI context together before network I/O, validate and persist the review before claiming delivery, and use atomic ownership tokens plus durable attempt counts to tolerate overlapping jobs and recovery. Never depend on separate workout-analysis completion. Recheck email opt-in before sending, include AI coaching in both HTML/text parts, and send statistics with a short unavailable message after terminal AI failure. Recover abandoned preparation and enqueue gaps periodically; ambiguous SMTP sends must become `uncertain` and require provider-log inspection before any resend. Never hold database locks during external calls or automatically retry an uncertain email.
    Weekly reviews default independently to `gpt-5.4-mini` with explicit medium reasoning, 25,000 output tokens (including reasoning), and a 180-second API timeout. Snapshot those request settings on creation alongside model/prompt version so retries preserve their original configuration; historical records retain legacy settings. Keep daily workout settings independent. Production SMTP uses explicit 10-second connection/30-second read deadlines rather than Rails' five-second defaults; longer deadlines must not bypass the uncertain-delivery guard.

50. **Admin AI Cost Contract**: Admin cost reporting estimates saved workout/weekly response usage with verified, exact-model USD token rates and an explicit reporting period/source date. Deduct cached input before charging uncached input and count reasoning within output only once. Include failed responses with usage; show missing usage and unknown pricing as unavailable rather than zero. Use indexed grouped SQL without loading private AI context or making API calls on page render. Explain coverage limits: saved response snapshots are not an invoice or a complete ledger of retry/interrupted charges.

51. **AI Completion Notification Contract**: Persist one notification per successful, current workout analysis in the result transaction. Use generic push text and link to that exact review version. Suppress push when any authenticated window has a current visible/focused presence lease, and recheck visibility/read status before claiming delivery; failed/superseded analyses never notify. Recover lost enqueue steps without repeating claimed pushes. Mark reviews read only through an authorized mutation after opening the notification or viewing the review on screen, never from a GET/frame fetch alone.
    Keep `/notifications/status` uncached and owner-scoped, returning 401 after sign-out. Service workers reconcile the unread badge on push, activation, background sync, notification interaction, and messages from app resume/read events. Preserve the badge on network failures; clear it for zero unread or sign-out. Workers cannot run continuously, and iOS requires each received push to show a notification: suppress foreground pushes server-side rather than sending silent badge pushes. Feature-detect the Badging API and preserve rest-timer delivery.
