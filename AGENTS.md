...
28. **Set Form Prefill Rules**: New set forms should prefill by strict order: (a) if current workout has prior sets for the exercise+machine, copy the immediately previous set; (b) otherwise copy set 1 from the previous finished session for that exact exercise+machine; (c) if no history exists, leave fields blank/default.
    ...
    The visible "Last" workout summary and any "last weight/reps" helper logic must use the most recent completed matching exercise+machine session and completion/position ordering within that session, not raw `started_at` / `created_at`. Exercise history pages should stay ordered by the visible workout date (`workouts.started_at DESC`) so tabs read in straightforward chronology, while still ordering sets within each session by completion/position. In exercise history, `machine_id` should choose the initially active tab when present, but should not hide other machine tabs or cross-machine history.
...
29. **Rest Timer Defaults**: The rest timer should always bootstrap from the user's configured `default_rest_seconds` (or the app fallback if none is set). Do not introduce per-block rest overrides into workout logging; block `rest_seconds` can remain as stored model data, but the active workout timer UX should use the user-level default consistently. When workout logging creates a brand new `WorkoutBlock`, still rely on `WorkoutBlock` model defaults instead of hardcoding `rest_seconds` in controllers.
    ...
    Final countdown pips should have a synchronized visual cue on the timer bar (forge orange on 4/2, back to normal on 3/1) while preserving the existing zero-second completion gradient sweep unchanged.
