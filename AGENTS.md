...
13. **Performance Alert Guardrails**: Volume-drop alerts should compare week-to-date volume against the same elapsed portion of last week and should not fire before any workout is logged in the current week. Fatigue analysis baselines must stay scoped to the exact exercise + machine combination.
    In tests for week-based alerts, anchor time with `travel_to` or explicit dates instead of relying on `N.days.ago`, so coverage stays stable across week boundaries.
...
32. **Workout Show Contracts**: Completed workout pages should not render add-exercise entry points. The workout page must keep the rest-timer stage-panel markup (`rest-timer-stage`, `rest-timer-panel`) present for smooth transitions, and completed workouts should surface grouped progression updates instead of in-form prompts.
