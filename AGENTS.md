...
29. **Rest Timer Defaults**: The rest timer should always bootstrap from the user's configured `default_rest_seconds` (or the app fallback if none is set). Do not introduce per-block rest overrides into workout logging; block `rest_seconds` can remain as stored model data, but the active workout timer UX should use the user-level default consistently. When workout logging creates a brand new `WorkoutBlock`, still rely on `WorkoutBlock` model defaults instead of hardcoding `rest_seconds` in controllers.
    ...
    Final countdown pips should have a synchronized forge-orange visual cue on the timer bar at 4/3/2/1 seconds while preserving the existing zero-second completion gradient sweep unchanged.
    ...
    Keep timer progress frame-synchronized and compositor-friendly: animate progress with transforms, confine cue/completion motion to overlays, avoid footer-wide animated filters/background positions, keep collapsed/active stage geometry stable, and disable decorative motion for `prefers-reduced-motion`. When timer targets are not mounted, fall back from animation-frame cadence so the layout-level controller does not consume 60fps off-page.
...
41. **Set Swipe Gesture Contract**: On touch devices, an intentional right swipe of a completed set row should duplicate it without requiring an edge-to-edge drag. Use a non-passive Touch Events path on iOS; Pointer Events can be cancelled by WebKit while the user is still swiping. Keep vertical page scrolling native, claim horizontal row gestures early enough for iOS Safari, reset cancellations below the action threshold, and commit cancellations after the duplicate threshold has been reached.
