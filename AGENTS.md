...
31. **Workout Block Header Layout**: On active workout logging screens, keep the block header compact and right-weighted: show the block number on the left, then keep the block rest controls and `+ S.Set` action grouped on the right as compact pill/round controls. Favor the block number over drag-handle clutter in the visible layout; the whole header can still act as the drag handle.
