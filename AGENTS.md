...
30. **iPhone PWA Navbar Safe Area**: Keep the main navbar explicitly `sticky` and, in standalone iPhone/home-screen mode, pad it by `env(safe-area-inset-top)` so it clears the status bar. The fixed offline indicator should use that same inset for its `top` offset.
    Modal and page-level action rows used in workout-setup flows must also respect `env(safe-area-inset-bottom)` so Back/Cancel controls never sit underneath iPhone browser or home-indicator chrome.
