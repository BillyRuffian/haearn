...
41. **Set Swipe Gesture Contract**: On touch devices, an intentional right swipe of a completed set row should duplicate it without requiring an edge-to-edge drag. Use captured Pointer Events as the primary modern-iOS path with legacy Touch Events only as a fallback. Keep vertical page scrolling native, claim horizontal row gestures early enough for iOS Safari, and reset cleanly when the browser cancels a gesture.
