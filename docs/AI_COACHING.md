...
6. The completed-workout **AI Coaching** panel subscribes to a signed, workout-specific Turbo Stream through the authenticated Action Cable connection. After a committed status change, the server pushes a small invalidation marker; Stimulus immediately reloads just the coaching Turbo Frame through the owner-scoped endpoint. This keeps advice and form tokens out of broadcasts and reads the latest saved state even if signals arrive out of order. Model callbacks cover creation/manual failures; atomic job transitions explicitly schedule their signal after commit. Rolled-back or superseded changes do not publish, and a broadcast outage cannot turn a saved successful analysis into a failure.
7. The stream subscription stays mounted outside the frame. Reconnection, returning to a visible tab, and going online fetch the latest panel; a signal received during an in-flight frame request schedules another fetch after that request finishes. There is no periodic browser polling. Pending/processing states show a compact rust-orange spinner while waiting for the push; **Refresh** remains available as a manual recovery action. Selecting a previous analysis pins that version against pushes and automatic refresh until the user chooses **Refresh** or navigates back to the latest view. The panel retains retry, reanalysis, and links to the ten latest versions; older records remain available through the scoped version endpoint and Rails console.
