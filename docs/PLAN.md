...
- [x] Make set-row swipe-right duplication reliable on iOS Safari with symmetric partial-reveal/full-action behavior, compatibility-click suppression, threshold-aware cancellation handling, and forced PWA cache refresh
...
- [x] Preserve higher internal kg precision so lb-entered exercise weights round-trip cleanly in forms and set displays
...
- [x] Keep exercise history views ordered by the visible workout date (`started_at`) and per-session set completion chronology, not raw `created_at`
